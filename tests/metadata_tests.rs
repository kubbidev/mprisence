...
fn create_extended_mpris_metadata() -> Metadata {
    ...
    data.insert(
        ...
        vec!["Rock".into(), "Metal".into()].into(),
    );
    data.insert(
        "xesam:artist".to_string(),
        vec!["Artist 1".into(), "Artist 2".into()].into(),
    );
...
fn test_metadata_extraction() {
    ...
    assert_eq!(
        metadata_source.artists().unwrap(),
        vec!["Artist 1".to_string(), "Artist 2".to_string()]
    );
    assert_eq!(metadata_source.track_number(), Some(1));
    assert_eq!(
        metadata_source.genres().unwrap(),
...
fn test_to_media_metadata_conversion() {
    ...
    assert_eq!(
        media_metadata.artists,
        vec!["Artist 1".to_string(), "Artist 2".to_string()]
    );
    assert_eq!(
        media_metadata.artist_display,
        Some("Artist 1, Artist 2".to_string())
    );
    assert_eq!(media_metadata.track_number, Some(1));
    assert_eq!(
        media_metadata.genres,
        vec!["Rock".to_string(), "Metal".to_string()]
    );
    assert_eq!(
        media_metadata.genre_display,
        Some("Rock, Metal".to_string())
    );
...
fn test_extended_metadata_fields() {
    ...
    assert_eq!(
        media_metadata.encoder_settings,
        Some("LAME 3.100".to_string())
    );
